...
When calling `new Bot()`, you may choose to pass an optional <Reference to="/docs/bot/types/BotPostOptions" /> object to configure the bot. This object can contain the following properties:
...
- `emitEvents`: Whether the bot should emit events. You'll learn more about this in [Listening to events](/guides/bot/event-handling/listening-to-events). Defaults to `true`.
- and various other configuration options — refer to the <Reference to="/docs/bot/types/BotPostOptions" /> documentation for the full list.
