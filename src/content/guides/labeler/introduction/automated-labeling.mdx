...
const post = await bot.post({ text: "Like the replies to this post to receive labels." });
...
The `post` method returns a <Reference to="/docs/bot/classes/PostReference" /> object, which you can use to reply to the post. The <Reference to="/docs/bot/classes/PostReference#reply" type="Method" /> method will return a reference to the reply.
