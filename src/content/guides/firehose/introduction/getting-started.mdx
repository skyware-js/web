...
`@skyware/firehose` is a utility library for consuming data from an [AT Protocol Relay](/guides/firehose/introduction/glossary#relay).
...
A Relay listens for changes to user [repositories](/guides/firehose/introduction/glossary#repository) and emits events when changes occur. You can listen for these events by calling the `on` method with a callback. Full documentation on the data you will receive with each event can be found in the <HighlightText kind={HighlightKind.Method} as="a" href="/docs/firehose/classes/Firehose#on" className="no-underline">Firehose#on</HighlightText> method documentation.
...
- `cid`: The [CID](/guides/firehose/introduction/glossary#cid) of the created or updated record.
